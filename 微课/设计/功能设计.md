...
| startDate      | Timestamp | 是   | 开始日期                                              |
| endDate        | Timestamp | 是   | 结束日期                                              |
| repeatType     | Integer   | 是   | 默认是0, 0 不重复、1 每天、 2 每周、3 自定义          |
| repeat         | Array     | 否   | 重复星期数集合, [0: 星期日、1星期一 ...]              |
...
| count          | int       | 是   | 消耗课时数                                            |
| status         | int       | 是   | 0：排课 1：消课 2：撤销 3. 编辑                       |
