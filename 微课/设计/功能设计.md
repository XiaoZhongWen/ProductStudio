...
| courseIds       | Array     | 否   | 课程id集合（wk-courses的主键id）                      |
| parentIds       | Array     | 否   | 家长id集合                                            |
| studentIds      | Array     | 否   | 学生id集合                                            |
| teacherIds      | Array     | 否   | 老师id集合                                            |
| classIds        | Array     | 否   | 班级id集合wk-classes的主键id）                        |
...
| type      | Integer   | 是   | 课程类型：0 一对一 ，1 班课 |
...
| studentId  | Object ID | 否   | 学生id（关联wk-users表的主键id）                           |
...
***注意：每个记录的teacherId、studentId、startTime、endTime不能完全一样；只有课程的授课老师和授课老师所在机构的管理员能编辑此表；只有与学生有关的机构、老师、家长及学生本人对此表有读的权限***
